...
std::map<std::string, Chassis::Transition> chassisPwrCtls = {
...
constexpr auto gpioDefFile = "/etc/default/obmc/gpio/gpio_defs.json";
...
Handler::Handler(sdbusplus::bus_t& bus) : bus(bus)
{
    ...
    for (const auto& gpioConfig : gpioDefs)
    {
        if (gpioConfig.contains("multi-action"))
        {
            ...
            {
                auto chassisPwrCtl =
                    chassisPwrCtls.find(ActCfg["action"]);
    ...
    try
    {
        ...
        if (!hostSelBtnMode && isMutiActSupport)
        {
            ...
            for (size_t countIter = 1; countIter < pwrBtnCount; countIter++)
            {
