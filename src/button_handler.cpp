...
void Handler::handlePowerEvent(PowerEvent powerEventType, std::chrono::microseconds duration)
{
    std::string objPathName;
    std::string dbusIfaceName;
    std::string transitionName;
    std::variant<Host::Transition, Chassis::Transition> transition;
    uint64_t durationMs = (duration.count() / 1000);
...
void Handler::powerReleased(sdbusplus::message_t& msg)
{
    try
    {
        ...
        handlePowerEvent(PowerEvent::powerReleased, std::chrono::microseconds(time));
    }
    catch (const sdbusplus::exception_t& e)
    {
...
void Handler::resetReleased(sdbusplus::message_t& /* msg */)
{
    try
    {
        ...
        handlePowerEvent(PowerEvent::resetReleased, std::chrono::microseconds(0));
    }
    catch (const sdbusplus::exception_t& e)
    {
