...
int configGroupGpio(buttonConfig& buttonIFConfig)
{
    ...
    for (auto& gpioCfg : buttonIFConfig.gpios)
    {
        ...
        if (result < 0)
        {
            lg2::error("{NAME}: Error configuring gpio: {NUM}", "NAME",
...
int configGpio(gpioInfo& gpioConfig)
{
    ...
    else
    {
        ...
        {
            lg2::error("{NUM} error in writing: {PATH}", "NUM", gpioNum, "PATH",
                       devPath);
            return -1;
        }
    }
    ...
    if (gpioDirection == "out")
    {
        ...
        {
            lg2::error("Error in reading: {PATH}", "PATH", devPath);
            return -1;
        }
