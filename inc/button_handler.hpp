...
{
    return std::accumulate(std::begin(INSTANCES), std::end(INSTANCES), 0,
                            [](auto m, auto s) {
                            return std::max(m, static_cast<int>(s)); });
}
...
class Handler
{
  ...
  private:
     ...
     * @return void
     */
    void handlePowerEvent(PowerEvent powerEventType, std::string objectPath, uint64_t duration);
